...
//   }    
  ...
  TFile *fmc = new TFile("ana_gj_out_job_qstar_700.root");
...
//       if (isData == 1  &&  hltbit == 4 ) continue;
     ...
     for(int ipho=0; ipho<nPho; ipho++){
        if(TMath::Abs(phoGenMomPID[ipho])>22) MC_type=1; else MC_type=0;
...
// 	 h_mgj_ptj[EBEE_type][MC_type]->Fill(gj_P4.M(), jetPt[ipho],ww);
   ...
   TFile *f = new TFile("ana_gj_out_job_qstar_700.root","recreate");
...
Int_t Ana_gj_mc::PhotonID(int ipho) {
...
Int_t Ana_gj_mc::PhotonID_3Iso(int ipho) {
...
Int_t Ana_gj_mc::jetID(int ijet){
...
Int_t Ana_gj_mc::phocat(int ipho) {
